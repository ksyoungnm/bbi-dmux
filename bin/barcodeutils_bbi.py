...
def parse_fastq_barcodes(r1, r2=None, spec=None, reverse_i5=False, edit_distance=2):
    ...
    for r in fastq_iterator:
        ...
        for barcode in spec:
            spec_barcode = spec[barcode]
            bc_read = spec_barcode[BC_READ]
            bc_start = spec_barcode[BC_START] - 1
            bc_end = spec_barcode[BC_END]
            ...
            if bc_read == I5:
                ...
                if reverse_i5:
                    if BC_WHITELIST in spec_barcode:
                        barcodes_dict[barcode] = spec_barcode[BC_CORRECTION_MAP].correct(reverse_complement(read_seq)[bc_start:bc_end])
                    else:
                        barcodes_dict[barcode] = reverse_complement(read_seq)[bc_start:bc_end]
                else:
                    if BC_WHITELIST in spec_barcode:
                        barcodes_dict[barcode] = spec_barcode[BC_CORRECTION_MAP].correct(read_seq[bc_start:bc_end])
                    else:
                        barcodes_dict[barcode] = read_seq[bc_start:bc_end]
                ...
                if BC_WHITELIST in spec_barcode:
                    barcodes_dict[barcode] = spec_barcode[BC_CORRECTION_MAP].correct(r1_seq[bc_start:bc_end])
                else:
                    barcodes_dict[barcode] = r1_seq[bc_start:bc_end]
                
            elif bc_read == R2:
                if not read_lengths_checked:
                    _validate_barcode_read_pair(r2_seq, bc_end)
                if BC_WHITELIST in spec_barcode:
                    barcodes_dict[barcode] = spec_barcode[BC_CORRECTION_MAP].correct(r2_seq[bc_start:bc_end])
                else:
                    barcodes_dict[barcode] = r2_seq[bc_start:bc_end]
